...
void InitResources(void)
{
    SnapShader = LoadShader("assets/snap.vs", 0);

    Grass = LoadTexture("assets/tile_0022.png");
    SetTextureFilter(Grass, TEXTURE_FILTER_POINT);

    Player = LoadTexture("assets/character_0004.png");
    SetTextureFilter(Player, TEXTURE_FILTER_POINT);

    Obstacle = LoadTexture("assets/tile_0026.png");
...
void UpdateDrawFrame(void)
{
    ...
    float screenOriginInCamera
        = floorf(GetScreenToWorld2D((Vector2) { 0, 0 }, ViewCamera).x);
    float screenEdgeInCamera = floorf(
        GetScreenToWorld2D(
