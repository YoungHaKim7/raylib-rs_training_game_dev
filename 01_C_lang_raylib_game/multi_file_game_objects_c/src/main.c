...
#include "headers/resource_dir.h" // utility header for SearchAndSetResourceDir
...
void InitScene()
{
    ...
    for (int index = 0; index < TheScene.ObjectCount; index++) {
        GameObjectAddComponent(
            TheScene.Objects + index, TransformComponent, CreateTransform());
        Transform2D* transform
            = GetTransformComponent(TheScene.Objects + index);
        transform->Position.x = (float)GetRandomValue(10, 1200);
        transform->Position.y = (float)GetRandomValue(10, 700);

        GameObjectAddComponent(TheScene.Objects + index, BehaviorComponent,
            CreateBahavior(UpdateTransform));

        float radius = (float)GetRandomValue(10, 30);
        GameObjectAddComponent(
            TheScene.Objects + index, ShapeComponent, CreateShape(radius));
        ...
        GameObjectAddComponent(
            child, BehaviorComponent, CreateBahavior(UpdateRotation));
    }
}
...
void DrawSprite(GameObject* object, Transform2D* transform)
{
    ...
    DrawTextureV(sprite->Texture,
        (Vector2) {
            sprite->Texture.width * 0.5f, sprite->Texture.height * 0.5f },
        WHITE);
